...
As proven empirically below across **174,925 loci of the T2T-CHM13 genome**, FastHumAS achieves **99.37% sensitivity, 99.96% precision, 100.00% strand concordance, and 99.28% label concordance** against the original HumAS-HMMER pipeline, while filtering out degenerate sub-monomer noise and fixing systematic coordinate offset defects in the legacy shell pipeline.
...
Across 9 finished nuclear chromosomes of T2T-CHM13 v2.0 (chr1, chr3, chr8, chr10, chr11, chr12, chr14, chr22, chrY), encompassing **174,925 legacy loci and 173,899 FastHumAS loci**, direct locus-by-locus benchmarking establishes:

| Metric | Measured Value | Biological Meaning & Assessment |
|---|:---:|---|
| **Sensitivity (Recall)** | **99.37%** | FastHumAS captures **173,827 of the 174,925** legacy loci. The Stage 1 Master Profile filter does not drop true alpha-satellite arrays. |
| **Precision** | **99.96%** | 173,827 of 173,899 FastHumAS calls are validated by legacy annotations (virtually zero spurious false positives). |
| **Strand Concordance** | **100.00%** | Zero polarity inversion errors across all 173,827 shared intervals. |
| **Subfamily / HOR Label Agreement** | **99.28%** | Exact model name assignment (e.g. `S1C10H1L.5`, `Ka`, `Ba`) matches character-for-character in 99.3% of cases. |
| **Boundary Concordance ($\le 3$ bp)** | **97.33%** | 169,194 monomers match within 1–3 bp; boundary shifts reflect intentional heuristic junction healing. |
| **Boundary Concordance ($\le 10$ bp)** | **99.72%** | 173,339 monomers match within 10 bp, confirming positional stability. |
| **Exact 0-bp Boundary Match** | **33.47%** | 58,183 monomers match bit-for-bit to the exact single nucleotide. |
...
### 4.1 Legacy-Unique Loci ($n = 1,098$, 0.63%)
Why did `nhmmer` detect 1,098 loci that FastHumAS did not output?
- **Length Distribution:** Mean length is only **104.5 bp** (median: 104.0 bp, range: 24–204 bp).
- **Score Distribution:** Mean score is **87.4**; 66.6% scored $<100$, and 94.4% scored $<150$.
- **Biological Nature:** These are **truncated sub-monomer fragments** located predominantly in degenerate pericentromeric transitions and transposable element borders. `nhmmer`'s local alignment picked up partial 30–80 bp motif matches that fail the full-monomer criteria of FastHumAS. FastHumAS correctly rejects these sub-monomers, preventing artificial fragmentation.

### 4.2 FastHumAS-Unique Loci ($n = 78$, 0.04%)
Why did FastHumAS detect 78 loci not present in the legacy output?
- **Length Distribution:** Mean length is **168.4 bp** (median: 169.0 bp), matching canonical 171-bp monomer architecture.
- **Score Distribution:** Mean score is **114.5** (range: 99.3–121.7).
- **Biological Nature:** These are **bona fide canonical monomers** that were discarded by the legacy pipeline due to the minus-strand coordinate offset bug (`aliFrom = $7 - 1; aliTo = $8`), which caused false boundary collisions during downstream `overlap_filter.py` execution.
...
### 5.1 The Minus-Strand 2-bp Offset Bug (~190,000–200,000 Gaps)
...
- **Empirical Quantification:** On the 9 evaluated chromosomes, legacy HumAS-HMMER contained 99,561 micro-gaps (1–3 bp). **68,211 of these (68.5%) were exact 2-bp gaps between adjacent minus-strand monomers**.
- **Genome-wide Extrapolation:** Across the ~240,000 minus-strand monomers in CHM13, this bug created **~190,000–200,000 artificial gaps**.
- **FastHumAS Fix:** FastHumAS uses symmetric interval projection:
  ...
  This eliminates 98.1% of artificial 2-bp minus-strand gaps (decreasing from 68,211 to 1,329).
...
- **Whole-Genome Acceptance:** FastHumAS evaluates 30,717 gaps in HOR and 62,723 in SF, accepting 35.1% and 55.7% of boundary extensions respectively. Gaps $>3$ bp and divergent transitions are strictly left open.
- **Flush Continuity:** The proportion of flush abutting junctions (gap = 0 bp) increases from **32.7% (57,220) to 93.0% (161,712)** across evaluated chromosomes.
- **Active Dense Arrays:** In the canonical active centromeric core of chromosome 22 (`NC_060946.1:12,788,180–15,711,065`, array `hor_22_9` / `S2C14/22H1L`, spanning 2.92 Mb with 17,145 monomers), **99.59% of junctions (17,073 / 17,144)** are resolved completely flush (gap = 0 bp), compared to **0.00% (0 / 17,150)** in legacy HumAS-HMMER where 96.38% (16,525) suffered from artificial micro-gaps. Genuine biological insertions and degenerate boundaries (only 65 micro-gaps and 6 interruptions >3 bp) are faithfully preserved. By contrast, in complex pericentromeric regions (e.g. `NC_060946.1:8,748,888–9,572,888`, which CenSat demonstrates is 89.5% non-alpha-satellite, composed of HSat3, bSat, and transitions), FastHumAS appropriately respects divergent boundaries.
...
- **High-Scoring Monomers ($\ge 150$ bits; $n = 80,341$, 98.4% of pairs):**
  - Mean ratio: $0.8507 \pm 0.0065$ (Median: $0.8517$)
- **Mid-Scoring Monomers (100–150 bits; $n = 1,320$, 1.6% of pairs):**
...
1. **Local Non-Maximum Suppression (`bedmap --max-element --fraction-either 0.1`):** Maps each reference interval to the highest-scoring candidate among all intervals sharing $\ge 10\%$ overlap with either element. When candidate scores are identical, FastHumAS selects the earliest candidate in `sort-bed` order `(chrom, start, end, name, strand)`, matching BEDOPS deterministic tie-breaking.
2. **Exact Deduplication (`exact_dedup`):** Eliminates duplicate records, matching legacy `awk '{if(!($0 in a)){a[$0]; print}}'`.
3. **Coordinate Order Restoration:** Because Stage 1 local maxima are emitted in reference order (which can momentarily invert adjacent coordinate order), FastHumAS explicitly sorts records before `near_dedup` and before returning from `filter_overlaps`. This ensures that downstream `junction::candidates` evaluates all true adjacent monomer junctions along the chromosome.
4. **Near-Duplicate Suppression (`near_dedup`):** Resolves near-identical records within $\pm 10$ bp, matching `overlap_filter.py`.
